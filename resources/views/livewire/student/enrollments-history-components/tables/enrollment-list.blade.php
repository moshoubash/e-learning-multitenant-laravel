<div class="divide-y-2 divide-on-surface/10">
    @forelse($enrollments as $enrollment)
        ...
        <div class="p-4 sm:p-[24px] hover:bg-surface-container-high transition-colors {{ $courseDeleted ? 'opacity-60' : '' }}">
            <div class="flex items-start gap-3 sm:gap-4">
                @if($courseDeleted)
                    <div class="flex items-center justify-center w-16 h-14 sm:w-20 sm:h-16 neo-border-sm neo-radius bg-surface-container shrink-0">
                        <i class="text-lg sm:text-xl text-secondary fas fa-ban"></i>
                    </div>
                @elseif($enrollment->course->thumbnail)
                    <img src="{{ $enrollment->course->thumbnail }}" alt="{{ $enrollment->course->title }}"
                        class="object-cover w-16 h-14 sm:w-20 sm:h-16 neo-border-sm neo-radius shrink-0">
                @else
                    <div class="flex items-center justify-center w-16 h-14 sm:w-20 sm:h-16 neo-border-sm neo-radius bg-surface-container shrink-0">
                        <i class="text-lg sm:text-xl text-secondary fas fa-book"></i>
                    </div>
                @endif
                <div class="flex-1 min-w-0">
                    <div class="flex items-start justify-between gap-2 sm:gap-4">
                        <div class="min-w-0 flex-1">
                            <h3 class="text-sm sm:text-base font-bold text-on-surface leading-tight">
                                {{ $courseDeleted ? __('messages.Course Unavailable') : $enrollment->course->title }}
                            </h3>
                            <p class="text-[10px] sm:text-xs text-secondary mt-0.5">
                        ...
                        @endphp
                        <span class="shrink-0 inline-flex items-center px-2 py-1 sm:px-2.5 text-[9px] sm:text-[10px] font-bold uppercase tracking-widest neo-border-sm {{ $class }}">
                            {{ __(ucfirst($enrollment->status)) }}
                        </span>
                    </div>
                    <div class="flex flex-wrap items-center gap-x-4 gap-y-1 mt-2 sm:mt-3 text-[10px] sm:text-xs text-secondary">
                        <span>
                    ...
                    <div class="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3 mt-2 sm:mt-3">
                        ...
                        @if(!$courseDeleted && ($enrollment->isActive() || $enrollment->isCompleted()))
                            <a href="{{ route('tenant.student.course', ['course' => $enrollment->course->slug]) }}"
                                class="inline-flex items-center justify-center px-3 py-1.5 text-[10px] font-bold uppercase tracking-widest neo-border-sm bg-primary-container text-on-primary-container hover:bg-on-surface hover:text-white transition-colors shrink-0">
    ...
    @empty
        <div class="p-6 sm:p-[24px] text-center py-8 sm:py-12">
