@if($showSectionDeleteModal && $deletingSection)
                ...
                <div class="px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    <div class="sm:flex sm:items-start">
                        ...
                        <div class="mt-3 text-center sm:mt-0 sm:ltr:ml-4 sm:rtl:mr-4 sm:ltr:text-left sm:rtl:text-right">
                            <h3 class="text-sm font-bold uppercase tracking-widest text-on-surface">{{ __('messages.Delete Section') }}</h3>
                            <p class="mt-2 text-sm text-secondary">
                                {{ __('messages.Are you sure you want to delete section :title?', ['title' => $deletingSection->title]) }}
                                {{ __('messages.This section and its lessons can be restored later.') }}
                            </p>
                        </div>
                    </div>
                </div>
                <div class="px-4 py-3 bg-surface-container-low sm:px-6 sm:flex sm:flex-row-reverse">
                    <button wire:click="softDeleteSection" type="button"
