        ...
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div
                ...
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    <h3 class="text-lg font-medium text-gray-900 mb-4">{{ __('messages.Edit Lesson') }}: {{ $editingLesson->title }}
                    </h3>
                    <form>
                        <div class="mb-4">
                            ...
                            @error('lessonEditTitle') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                        </div>
                        <div class="mb-4">
                            <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('messages.Type') }}</label>
                            <select wire:model.lazy="lessonEditType"
                                class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
                                <option value="video" {{ $editingLesson->type === 'video' ? 'selected' : '' }}>Video</option>
                                <option value="text" {{ $editingLesson->type === 'text' ? 'selected' : '' }}>Text</option>
                                <option value="quiz" {{ $editingLesson->type === 'quiz' ? 'selected' : '' }}>Quiz</option>
                            </select>
                            @error('lessonEditType') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                        </div>
                        <div class="mb-4">
                            ...
                            <div class="mt-2 bg-white" wire:ignore>
                                ...
                                @error('lessonEditContent') <span class="text-red-500 text-xs">{{ $message }}</span>
                                @enderror
                            </div>
                        </div>
                        <div class="mb-4">
                            <label
                                class="block text-sm font-medium text-gray-700 mb-1">{{ __('messages.Duration (seconds)') }}</label>
                            <input type="number" wire:model.lazy="lessonEditDuration" min="0"
                                class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
                            @error('lessonEditDuration') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                        </div>
                        <div class="mb-4">
                        ...
                        <div class="mb-4" x-show="$wire.lessonEditType === 'video'">
                            <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('messages.Video URL') }}</label>
                            ...
                            @error('lessonEditVideoUrl') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                        </div>
                    </form>
                </div>
