        ...
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div class="inline-block overflow-hidden text-left align-bottom transition-all transform bg-white rounded-lg shadow-xl sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                <div class="px-4 pt-5 pb-4 bg-white sm:p-6 sm:pb-4">
                    <h3 class="mb-4 text-lg font-medium text-gray-900">{{ __('messages.Restore Assignment') }}</h3>
                    <p class="text-sm text-gray-600">{{ __('messages.This assignment has been deleted. You can restore it to bring it back to the section.') }}</p>
                    <div class="mt-4 p-4 bg-gray-50 rounded-lg">
                        <p class="font-medium text-gray-800">{{ $restoringAssignment->title }}</p>
                        <p class="text-sm text-gray-500">{{ Str::limit($restoringAssignment->description, 120) }}</p>
                    </div>
                </div>
