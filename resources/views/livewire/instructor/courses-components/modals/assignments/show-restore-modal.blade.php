@if($showAssignmentRestoreModal && $restoringAssignment)
        ...
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div class="inline-block overflow-hidden ltr:text-left rtl:text-right align-bottom transition-all transform bg-surface-container-lowest neo-border neo-radius sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                <div class="px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    <h3 class="text-sm font-bold uppercase tracking-widest text-on-surface mb-4">{{ __('messages.Restore Assignment') }}</h3>
                    <p class="text-sm text-secondary">{{ __('messages.This assignment has been deleted. You can restore it to bring it back to the section.') }}</p>
                    <div class="mt-4 p-4 neo-border-sm neo-radius bg-surface-container-low">
                        <p class="font-bold text-on-surface">{{ $restoringAssignment->title }}</p>
                        <p class="text-sm text-secondary mt-1">{{ Str::limit($restoringAssignment->description, 120) }}</p>
                    </div>
                </div>
                <div class="px-4 py-3 bg-surface-container-low sm:px-6 sm:flex sm:flex-row-reverse">
                    <button wire:click="restoreAssignment" type="button"
