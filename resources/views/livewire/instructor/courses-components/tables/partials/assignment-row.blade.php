    ...
    <div class="flex items-center justify-between text-sm">
        ...
        <div class="flex items-center gap-2">
            <button wire:click="openAssignmentEditModal({{ $assignment->id }})"
                class="flex items-center justify-center transition-colors w-8 h-8 neo-border-sm neo-radius text-on-surface hover:bg-primary-container hover:text-on-primary-container" title="Edit Assignment">
                <i class="text-xs fas fa-edit"></i>
            </button>
            @if($assignment->deleted_at)
                <button wire:click="openAssignmentRestoreModal({{ $assignment->id }})"
                    class="flex items-center justify-center transition-colors w-8 h-8 neo-border-sm neo-radius text-on-surface hover:bg-primary-container hover:text-on-primary-container" title="Restore Assignment">
                    <i class="text-xs fas fa-undo"></i>
                </button>
            @else
                <button wire:click="openAssignmentDeleteModal({{ $assignment->id }})"
                    class="flex items-center justify-center transition-colors w-8 h-8 neo-border-sm neo-radius text-error hover:bg-error hover:text-white" title="Delete Assignment">
