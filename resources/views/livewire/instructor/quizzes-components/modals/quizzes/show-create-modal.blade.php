        ...
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div class="inline-block overflow-hidden ltr:text-left rtl:text-right align-bottom transition-all transform bg-surface-container-lowest neo-border neo-radius sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                <div class="px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    <h3 class="text-sm font-bold uppercase tracking-widest text-on-surface mb-4">{{ __('messages.Create New Quiz') }}</h3>
                    <form>
                        <div class="mb-4">
                        ...
                        <div class="mb-4">
                            <label class="block mb-1 text-xs font-bold uppercase tracking-widest text-on-surface">{{ __('messages.Section') }}</label>
                            <select wire:model.lazy="quizCreateSectionId"
                                ...
                                @foreach($sections as $section)
                                    <option value="{{ $section->id }}">{{ $section->title }} - {{ $section->course->title ?? 'No Course' }}</option>
                                @endforeach
                            </select>
                            @error('quizCreateSectionId') <span class="text-xs text-error mt-1 block font-bold">{{ $message }}</span> @enderror
                        </div>
                        <div class="mb-4">
                        ...
                        <div class="mb-4">
                            ...
                            <p class="mt-1 text-xs text-secondary ltr:ml-6 rtl:mr-6">{{ __('messages.Allow students to retake this quiz after passing') }}</p>
                        </div>
                    </form>
                </div>
