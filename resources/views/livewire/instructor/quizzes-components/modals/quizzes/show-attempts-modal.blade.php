@if($showAttemptsModal && $attemptsQuizId)
    ...
    <div class="fixed inset-0 z-50 overflow-y-auto" x-data="{ show: true }" x-show="show">
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
            <div class="fixed inset-0 bg-on-surface/60 transition-opacity" wire:click="closeAttemptsModal"></div>
                ...
                <div class="px-4 pt-5 pb-4 sm:p-6 sm:pb-4 max-h-[90vh] overflow-y-auto">
                            ...
                            <table class="w-full ltr:text-left rtl:text-right">
                                <thead class="bg-surface-container-low border-b-2 border-on-surface">
                                    <tr>
                                        ...
                                        <th class="p-3 text-[10px] font-bold uppercase tracking-widest text-secondary">{{ __('messages.Email') }}</th>
                                ...
                                <tbody class="divide-y divide-[#E5E5E5]">
                                    @foreach($attempts as $attempt)
                                        <tr class="hover:bg-surface-container-low transition-colors">
                                            ...
                                            <td class="p-3 text-sm text-secondary">{{ $attempt->user?->email ?? 'N/A' }}</td>
                                            <td class="p-3">
                                            ...
                                            <td class="p-3 text-sm text-secondary">{{ $attempt->submitted_at ? $attempt->submitted_at->isoformat('dddd, D MMMM YYYY') : 'N/A' }}</td>
                                        </tr>
                                    @endforeach
                                </tbody>
                            </table>
                    ...
                    @else
                        <div class="py-12 text-center">
                            ...
                            <p class="text-sm text-secondary">{{ __('messages.No student attempts yet for this quiz.') }}</p>
                        </div>
                    @endif
                </div>
