<div>
    <header class="h-16 flex justify-between items-center px-[24px] bg-surface-container-lowest border-b-2 border-on-surface sticky top-0 z-40">
        <div>
            <h2 class="text-[24px] font-bold text-on-surface leading-none tracking-[0.08em]">{{ __('messages.Enrollments') }}</h2>
        <p class="text-[12px] font-medium uppercase text-secondary mt-0.5 tracking-wider">{{ __('messages.View all student enrollments across the platform') }}</p>
    </div>
                ...
                <table class="w-full text-sm">
                    <thead>
                        <tr class="border-b-2 border-on-surface bg-surface-container">
                            ...
                            <th class="ltr:text-left rtl:text-right px-[24px] py-3 text-xs font-bold uppercase tracking-widest text-on-surface">{{ __('messages.Instructor') }}</th>
                    ...
                    <tbody class="divide-y-2 divide-on-surface/10">
                        @forelse($enrollments as $enrollment)
                            <tr class="hover:bg-surface-container-high transition-colors">
                                ...
                                <td class="px-[24px] py-3">
                                    <span class="text-secondary">{{ $enrollment->course->instructor->name ?? '—' }}</span>
                                </td>
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="px-[24px] py-12 text-center">
