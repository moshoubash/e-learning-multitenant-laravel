        ...
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div class="inline-block overflow-hidden align-bottom transition-all transform ltr:text-left rtl:text-right bg-surface-container-lowest neo-border neo-radius sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                <div class="px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    <h3 class="mb-4 text-sm font-bold tracking-widest uppercase text-on-surface">{{ __('messages.Create New Course') }}</h3>
                    <form>
                        <div class="mb-4">
                            ...
                            @error('createTitle') <span class="block mt-1 text-xs font-bold text-error">{{ $message }}</span> @enderror
                        </div>
                        <div class="mb-4">
                            <label class="block mb-1 text-xs font-bold tracking-widest uppercase text-on-surface">{{ __('messages.Description') }}</label>
                            <textarea wire:model.lazy="createDescription" rows="3"
                                class="w-full px-3 py-2 text-sm neo-border-sm neo-radius bg-surface-container-low text-on-surface focus:outline-none focus:ring-0 placeholder:text-secondary"></textarea>
                            @error('createDescription') <span class="block mt-1 text-xs font-bold text-error">{{ $message }}</span> @enderror
                        </div>
                        <div class="mb-4">
                            <label class="block mb-1 text-xs font-bold tracking-widest uppercase text-on-surface">{{ __('messages.Instructor') }}</label>
                            <select wire:model.lazy="createInstructorId"
                        ...
                        <div class="mb-4">
                            ...
                            <input type="number" step="0.01" min="0" wire:model.lazy="createPrice"
                                class="w-full px-3 py-2 text-sm neo-border-sm neo-radius bg-surface-container-low text-on-surface focus:outline-none focus:ring-0 placeholder:text-secondary">
                            @error('createPrice') <span class="block mt-1 text-xs font-bold text-error">{{ $message }}</span> @enderror
                        </div>
                        <div class="mb-4">
                            <label class="block mb-1 text-xs font-bold tracking-widest uppercase text-on-surface">{{ __('messages.Status') }}</label>
                            ...
                            @error('createStatus') <span class="block mt-1 text-xs font-bold text-error">{{ $message }}</span> @enderror
                        </div>
                        <div class="mb-4">
                            <label class="block mb-1 text-xs font-bold tracking-widest uppercase text-on-surface">{{ __('messages.Thumbnail') }}</label>
                            <input type="file" wire:model.lazy="createThumbnail" accept="image/jpg,image/jpeg,image/png,image/webp"
                            ...
                            @error('createThumbnail') <span class="block mt-1 text-xs font-bold text-error">{{ $message }}</span> @enderror
                            <div wire:loading wire:target="createThumbnail" class="mt-2 text-xs font-bold text-on-surface">
                                <span>{{ __('messages.Uploading...') }}</span>
                            </div>
                            @if ($createThumbnail)
                                <div class="mt-2">
                                    <img src="{{ $createThumbnail->temporaryUrl() }}" class="w-20 h-20 object-cover neo-border-sm neo-radius">
                                </div>
                            @endif
                        </div>
                    </form>
