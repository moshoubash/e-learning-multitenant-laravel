<div>
    <header class="h-16 flex justify-between items-center px-[24px] bg-surface-container-lowest border-b-2 border-on-surface sticky top-0 z-40">
        <div>
            <h2 class="text-[24px] font-bold text-on-surface leading-none tracking-[0.08em]">{{ __('messages.SMTP Configuration') }}</h2>
    ...
    <div class="p-[24px] max-w-[800px] mx-auto">
        <div class="overflow-hidden bg-surface-container-lowest neo-border neo-radius">
            <div class="p-[24px] border-b-2 border-on-surface">
                ...
                <p class="text-xs text-secondary mt-2">{{ __('messages.These settings will override the .env mail configuration for this tenant. Leave fields empty to use defaults.') }}</p>
            </div>

            <form wire:submit="save" class="p-[24px] space-y-6">
                <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                    <div>
                        <label class="block mb-1 text-xs font-bold uppercase tracking-widest text-on-surface">{{ __('messages.Mailer') }}</label>
                        ...
                        @error('mailMailer') <span class="text-xs text-error mt-1 block font-bold">{{ $message }}</span> @enderror
                    </div>

                    <div>
                        <label class="block mb-1 text-xs font-bold uppercase tracking-widest text-on-surface">{{ __('messages.Encryption') }}</label>
                        ...
                        @error('mailEncryption') <span class="text-xs text-error mt-1 block font-bold">{{ $message }}</span> @enderror
                    </div>

                    <div>
                        <label class="block mb-1 text-xs font-bold uppercase tracking-widest text-on-surface">{{ __('messages.Host') }}</label>
                        ...
                        @error('mailHost') <span class="text-xs text-error mt-1 block font-bold">{{ $message }}</span> @enderror
                    </div>

                    <div>
                        <label class="block mb-1 text-xs font-bold uppercase tracking-widest text-on-surface">{{ __('messages.Port') }}</label>
                        <input type="number" wire:model.lazy="mailPort" placeholder="587"
                            class="w-full px-3 py-2 text-sm neo-border-sm neo-radius bg-surface-container-low text-on-surface focus:outline-none focus:ring-0 placeholder:text-secondary">
                        @error('mailPort') <span class="text-xs text-error mt-1 block font-bold">{{ $message }}</span> @enderror
                    </div>

                    <div>
                        <label class="block mb-1 text-xs font-bold uppercase tracking-widest text-on-surface">{{ __('messages.Username') }}</label>
                        <input type="text" wire:model.lazy="mailUsername" placeholder="user@gmail.com"
                            class="w-full px-3 py-2 text-sm neo-border-sm neo-radius bg-surface-container-low text-on-surface focus:outline-none focus:ring-0 placeholder:text-secondary">
                        @error('mailUsername') <span class="text-xs text-error mt-1 block font-bold">{{ $message }}</span> @enderror
                    </div>

                    <div>
                        <label class="block mb-1 text-xs font-bold uppercase tracking-widest text-on-surface">{{ __('messages.Password') }}</label>
                        ...
                        @error('mailPassword') <span class="text-xs text-error mt-1 block font-bold">{{ $message }}</span> @enderror
                    </div>

                    <div>
                        <label class="block mb-1 text-xs font-bold uppercase tracking-widest text-on-surface">{{ __('messages.From Address') }}</label>
                        <input type="email" wire:model.lazy="mailFromAddress" placeholder="noreply@example.com"
                            class="w-full px-3 py-2 text-sm neo-border-sm neo-radius bg-surface-container-low text-on-surface focus:outline-none focus:ring-0 placeholder:text-secondary">
                        @error('mailFromAddress') <span class="text-xs text-error mt-1 block font-bold">{{ $message }}</span> @enderror
                    </div>

                    <div>
                        <label class="block mb-1 text-xs font-bold uppercase tracking-widest text-on-surface">{{ __('messages.From Name') }}</label>
                        <input type="text" wire:model.lazy="mailFromName" placeholder="GRID LMS"
                            class="w-full px-3 py-2 text-sm neo-border-sm neo-radius bg-surface-container-low text-on-surface focus:outline-none focus:ring-0 placeholder:text-secondary">
                        @error('mailFromName') <span class="text-xs text-error mt-1 block font-bold">{{ $message }}</span> @enderror
                    </div>
                </div>
                ...
                <div class="flex items-center gap-3 pt-4 border-t border-on-surface/10">
                    <div class="flex items-center gap-2">
                        ...
                        <label for="isActive" class="text-xs font-bold uppercase tracking-widest text-on-surface cursor-pointer">{{ __('messages.Active') }}</label>
                    </div>
                </div>
