...
<aside class="fixed ltr:left-0 rtl:right-0 top-0 h-full w-[240px] hidden lg:flex flex-col bg-surface-container-lowest ltr:border-r-2 rtl:border-l-2 border-on-surface z-50">
    ...
    <nav class="flex-1 p-4 space-y-2 overflow-y-auto no-scrollbar"
         x-data x-init="$el.scrollTop = sessionStorage.getItem('sidebar-scroll-instructor') ?? 0"
         @scroll="sessionStorage.setItem('sidebar-scroll-instructor', $el.scrollTop)">
    ...
    <div class="p-4 space-y-4 border-t-2 border-on-surface">
        ...
        <div class="flex items-center gap-3">
            ...
            <div class="flex-1">
                ...
                <p class="text-xs text-secondary">Instructor</p>
            </div>
            <div class="flex items-center gap-1">
                <form method="POST" action="{{ route('logout') }}" class="inline" onsubmit="sessionStorage.removeItem('sidebar-scroll-instructor')">
                    @csrf
